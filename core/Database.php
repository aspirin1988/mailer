...
class Database
{
    ...
    {
        $str='';
        !substr_count($string,'.*')? $str = '"' . str_replace('.', '"."', preg_replace('/(^#|\(JSON\)\s*)/', '', $string)) . '"' :
        $str ='"' . str_replace('.', '".', preg_replace('/(^#|\(JSON\)\s*)/', '', $string)) . '';
        return $str; //'"' . str_replace('.', '"."', preg_replace('/(^#|\(JSON\)\s*)/', '', $string)) . '"';
    }
    ...
    public function insert($table, $datas)
    {
        ...
        foreach ($datas as $data) {
            ...
            $lastId[] = ($this->database_type == 'pgsql') ? $this->pdo->lastInsertId($table . '_id_seq') : $this->pdo->lastInsertId();
    ...
    {
        $path = LOG_PATH . DS . 'db.txt';
        $message .= "\n";
        $message .= date('Y-m-d G:i:s');
        $message .= "\n";
        $message .= print_r($this->error(), true);
        $message .= "\n\n";
